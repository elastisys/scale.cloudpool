...
if [ "${LOG_DIR}" = "" ]; then
    LOG_DIR="/var/log/elastisys/citycloudpool"
fi

# where runtime state is kept
if [ "${STORAGE_DIR}" = "" ]; then
    STORAGE_DIR="/var/lib/elastisys/citycloudpool"
fi

logconfig="/etc/elastisys/citycloudpool/logback.xml"
julconfig="/etc/elastisys/citycloudpool/logging.properties"
...
# have been set up
authbind --deep ${java} ${JAVA_OPTS} -jar citycloudpool.jar ${SERVER_OPTS}
