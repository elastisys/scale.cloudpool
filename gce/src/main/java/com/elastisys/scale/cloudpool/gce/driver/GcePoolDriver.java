...
import com.elastisys.scale.cloudpool.gce.util.InstanceUrl;
import com.elastisys.scale.cloudpool.gce.util.MetadataUtil;
import com.elastisys.scale.cloudpool.gce.util.UrlUtils;
import com.elastisys.scale.cloudpool.gce.util.ZoneUtils;
 ...
 */
public class GcePoolDriver implements CloudPoolDriver {
    private static final Logger LOG = LoggerFactory.getLogger(GcePoolDriver.class);
    ...
    private final GceClient client;
    ...
    public GcePoolDriver(GceClient client) {
        this.client = client;
    }
    ...
    public List<Machine> listMachines() throws IllegalStateException, CloudPoolDriverException {
        ...
        for (ManagedInstance member : members) {
            ...
            Instance instance = this.client.getInstance(provisioningTemplate().getProject(), instanceUrl.getZone(),
                    instanceUrl.getName());
        ...
        if (numAwaiting > 0) {
            ...
            InstanceTemplate instanceTemplate = this.client.getInstanceTemplate(provisioningTemplate().getProject(),
                    UrlUtils.basename(instanceGroup.getInstanceTemplate()));
    ...
    public void setServiceState(String instanceUrl, ServiceState serviceState)
        ...
        this.client.setMetadata(url.getProject(), url.getZone(), url.getName(),
    ...
    private Instance getGroupMember(String instanceUrl) throws NotFoundException {
        ...
        for (ManagedInstance member : members) {
            if (member.getInstance().equals(instanceUrl)) {
                InstanceUrl memberUrl = new InstanceUrl(member.getInstance());
                return this.client.getInstance(memberUrl.getProject(), memberUrl.getZone(), memberUrl.getName());
            }
        }
    ...
    InstanceGroupClient instanceGroupClient() {
        ...
        if (provisioningTemplate.isSingleZoneGroup()) {
            return this.client.singleZoneInstanceGroup(provisioningTemplate.getProject(),
                    provisioningTemplate.getZone(), provisioningTemplate.getInstanceGroup());
        } else {
            return this.client.multiZoneInstanceGroup(provisioningTemplate.getProject(),
