...
public class FakeSingleZoneInstanceGroup {
    ...
    public InstanceGroupManager instanceGroupManager() {
        String instanceTemplateUrl = String.format(
                "https://www.googleapis.com/compute/v1/projects/%s/zones/%s/instanceTemplates/%s", project(), zone(),
                INSTANCE_TEMPLATE_NAME);
        String instanceGroupUrl = String.format(
                "https://www.googleapis.com/compute/v1/projects/%s/zones/%s/instanceGroups/%s", project(), zone(),
                BASE_INSTANCE_NAME);
