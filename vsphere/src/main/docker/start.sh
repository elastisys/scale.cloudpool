...
# Logging
LOG_CONFIG=${LOG_CONFIG:-/etc/elastisys/vspherepool/logback.xml}
JUL_CONFIG=${JUL_CONFIG:-/etc/elastisys/vspherepool/logging.properties}
# log destination dir for default LOG_CONFIG
LOG_DIR=${LOG_DIR:-/var/log/elastisys/vspherepool}
mkdir -p ${LOG_DIR}
...
STORAGE_DIR=${STORAGE_DIR:-/var/lib/elastisys/vspherepool}
...
if [ "${MULTIPOOL}" = true ]; then
    ${java} ${JVM_OPTS} ${JAVA_OPTS} -cp /opt/elastisys/vspherepool/vspherepool.jar com.elastisys.scale.cloudpool.vsphere.server.multipool.Main ${RUNTIME_OPTS} ${SERVER_OPTS} ${AUTH_OPTS}
else
    ${java} ${JVM_OPTS} ${JAVA_OPTS} -jar /opt/elastisys/vspherepool/vspherepool.jar ${RUNTIME_OPTS} ${SERVER_OPTS} ${AUTH_OPTS}
fi
