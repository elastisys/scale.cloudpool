...
public class TestAuthConfig {
    ...
    public void configureCertAuthByPath() throws Exception {
        ...
        assertThat(config.getClientCert(), is(loadCert(CLIENT_CERT_PATH)));
        assertThat(config.getClientKey(), is(loadKey(CLIENT_KEY_PATH)));
    ...
    public void configureCertAuthByValue() throws Exception {
        AuthConfig config = AuthConfig.builder().cert(loadString(CLIENT_CERT_BASE64_PATH))
                .key(loadString(CLIENT_KEY_BASE64_PATH)).build();
        config.validate();
    ...
    public void configureCertByPathAndKeyByValue() throws Exception {
        ...
        AuthConfig config = AuthConfig.builder().certPath(CLIENT_CERT_PATH).key(loadString(CLIENT_KEY_BASE64_PATH))
                .build();
        assertThat(config.getClientCert(), is(loadCert(CLIENT_CERT_PATH)));
        assertThat(config.getClientKey(), is(loadKey(CLIENT_KEY_PATH)));

        // client by value, key by path
        config = AuthConfig.builder().cert(loadString(CLIENT_CERT_BASE64_PATH)).keyPath(CLIENT_KEY_PATH).build();
        assertThat(config.getClientCert(), is(loadCert(CLIENT_CERT_PATH)));
        assertThat(config.getClientKey(), is(loadKey(CLIENT_KEY_PATH)));
    }
    ...
    public void specifyClientKeyBothAsPathAndValue() throws Exception {
        AuthConfig.builder().certPath(CLIENT_CERT_PATH).keyPath(CLIENT_KEY_PATH).key(loadString(CLIENT_KEY_PATH))
                .build().validate();
    }

    @Test(expected = IllegalArgumentException.class)
    public void specifyClientCertBothAsPathAndValue() throws Exception {
        AuthConfig.builder().certPath(CLIENT_CERT_PATH).cert(loadString(CLIENT_CERT_PATH)).keyPath(CLIENT_KEY_PATH)
                .build().validate();
    }
    ...
    public void configureTokenAuthByValue() throws Exception {
        AuthConfig config = AuthConfig.builder().token(loadString(CLIENT_TOKEN_PATH)).build();
        config.validate();
    ...
    public void specifyClientTokenBothAsPathAndValue() throws Exception {
        AuthConfig.builder().tokenPath(CLIENT_TOKEN_PATH).token(loadString(CLIENT_TOKEN_PATH)).build().validate();
    }
    ...
    public void configureServerAuthByPath() throws Exception {
        ...
        assertThat(config.getServerCert(), is(loadCert(SERVER_CERT_PATH)));
    }
    ...
    public void configureServerAuthByValue() throws Exception {
        AuthConfig config = AuthConfig.builder().certPath(CLIENT_CERT_PATH).keyPath(CLIENT_KEY_PATH)
                .serverCert(loadString(SERVER_CERT_BASE64_PATH)).build();
        config.validate();
