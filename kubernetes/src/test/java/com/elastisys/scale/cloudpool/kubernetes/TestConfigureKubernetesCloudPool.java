...
public class TestConfigureKubernetesCloudPool {
    ...
    public void configWithDefaults() throws Exception {
        ...
        assertThat(this.cloudPool.config().getApiServer().getPort(), is(ApiServerConfig.DEFAULT_PORT));
        assertThat(this.cloudPool.config().getApiServer().getHost(), is(ApiServerConfig.DEFAULT_HOST));
    ...
    public void setTokenAuthByPathConfig() throws Exception {
        ...
        assertThat(this.cloudPool.config().getAuth().getClientToken(), is(loadToken("auth-token.base64")));
        assertThat(this.cloudPool.config().getAuth().getServerCert(), is(loadCert("ca.pem")));
    }
    ...
    public void certSpecifiedBothByPathAndContent() throws Exception {
        JsonObject config = loadConfig("invalid-config-cert-path-and-content.json");
        this.cloudPool.configure(config);
    }
    ...
    public void configWithNonExistentKeyPath() {
        JsonObject config = loadConfig("invalid-config-nonexistent-key-path.json");
        this.cloudPool.configure(config);
    }
