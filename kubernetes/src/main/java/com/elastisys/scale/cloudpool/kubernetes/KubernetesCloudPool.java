...
public class KubernetesCloudPool implements CloudPool {
	private static final Logger LOG = LoggerFactory
			.getLogger(KubernetesCloudPool.class);
	private static final CloudPoolMetadata METADATA = new CloudPoolMetadata(
			PoolIdentifiers.KUBERNETES, Arrays.asList(ApiVersion.LATEST));
	...
	public KubernetesCloudPool(KubernetesClient apiClient) {
		...
		ThreadFactory threadFactory = new ThreadFactoryBuilder()
				.setNameFormat("pool-updater-%d").setDaemon(true).build();
		this.executor = new StandardRestartableScheduledExecutorService(
				MAX_THREADS, threadFactory);
	}

	@Override
	public synchronized void configure(JsonObject configuration)
		...
		LOG.info("applying new configuration: {}",
				JsonUtils.toPrettyString(configuration));
	...
	private KubernetesCloudPoolConfig parseAndValidate(
			JsonObject configuration) {
		KubernetesCloudPoolConfig newConfig = null;
		try {
			newConfig = JsonUtils.toObject(configuration,
					KubernetesCloudPoolConfig.class);
			newConfig.validate();
		} catch (Exception e) {
			throw new IllegalArgumentException(
					String.format("invalid config: %s", e.getMessage()), e);
		}
		return newConfig;
	}
	...
	public Optional<JsonObject> getConfiguration() {
		...
		return Optional
				.fromNullable(JsonUtils.toJson(this.config).getAsJsonObject());
	}
	...
	public void start() throws NotConfiguredException {
		...
		this.executor.scheduleWithFixedDelay(
				new PoolUpdateTask(this.kubernetesClient, this.desiredSize),
	...
	private void tryToDetermineDesiredSize() {
		...
		try {
			...
			this.desiredSize
					.set(this.kubernetesClient.getPoolSize().getDesiredSize());
	...
	public void stop() {
		...
		try {
			...
			LOG.warn("interrupted while waiting for tasks to stop: {}",
					e.getMessage());
		}
	...
	public MachinePool getMachinePool()
			throws CloudPoolException, NotStartedException {
		ensureStarted();
		return this.kubernetesClient.getMachinePool();
	}

	@Override
	public PoolSizeSummary getPoolSize()
			throws CloudPoolException, NotStartedException {
		ensureStarted();
	...
	@Override
	public void setDesiredSize(int desiredSize) throws IllegalArgumentException,
	...
	private void throwUnsupportedOperation() {
		throw new UnsupportedOperationException(String.format(
				"this operation is not supported by the %s", DESCRIPTION));
	}
	...
	private void ensureConfigured() throws IllegalStateException {
		if (this.config == null) {
			throw new IllegalStateException(
					"cloudpool has not been configured");
		}
	}
	...
	private void ensureStarted() throws NotStartedException {
		if (!isStarted()) {
			throw new NotStartedException(
					"cannot use cloudpool in stopped state");
		}
	}
	...
	private static class PoolUpdateTask implements Runnable {
		...
		public PoolUpdateTask(KubernetesClient apiClient,
				AtomicReference<Integer> desiredSize) {
