...
public class AuthenticatingHttpApiClient implements HttpApiClient {
    private static final Logger LOG = LoggerFactory.getLogger(AuthenticatingHttpApiClient.class);
    ...
    public void configure(AuthConfig config) throws IllegalArgumentException {
        LOG.debug("new auth config set: {}", JsonUtils.toPrettyString(JsonUtils.toJson(config)));
        this.config = config;
    }

    @Override
    public JsonObject get(String url) throws HttpResponseException, IOException {
    ...
    @Override
    public JsonObject patch(String url, JsonObject data) throws HttpResponseException, IOException {
        Http client = buildHttpClient();
        HttpPatch patch = new HttpPatch(url);
        patch.addHeader("Content-Type", "application/merge-patch+json");
        patch.setEntity(new StringEntity(JsonUtils.toPrettyString(data), Charsets.UTF_8));
    ...
    private void ensureConfigured() throws IllegalStateException {
        if (this.config == null) {
            throw new IllegalStateException("attempt to use http client before being configured");
        }
    }
     ...
     * @return
     */
    private Http buildHttpClient() throws KubernetesApiException, IllegalStateException {
        ensureConfigured();
        ...
        try {
            ...
            if (auth.hasClientCert()) {
                ...
                KeyStore clientCertKeystore = PemUtils.keyStoreFromCertAndKey(auth.getClientCert(), auth.getClientKey(),
                        keyPassword);
            ...
            if (auth.hasServerCert()) {
                ...
                KeyStore trustStore = PemUtils.keyStoreFromCert(auth.getServerCert());
