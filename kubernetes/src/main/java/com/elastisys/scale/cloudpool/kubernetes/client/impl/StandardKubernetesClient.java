...
public class StandardKubernetesClient implements KubernetesClient {
    private static final Logger LOG = LoggerFactory.getLogger(StandardKubernetesClient.class);
    ...
    private void ensureConfigured() throws IllegalStateException {
        if (this.config == null) {
            throw new IllegalStateException("kubernetes apiserver client not configured yet");
        }
    }
     ...
     */
    private JsonObject ensureReplicationController() throws KubernetesApiException {
        try {
            return getReplicationController();
        } catch (Exception e) {
            throw new KubernetesApiException(String.format("cannot complete API request: the "
                    + "configured replication controller " + "could not be reached: %s", e.getMessage()), e);
        }
    }
    ...
    public PoolSizeSummary getPoolSize() throws KubernetesApiException {
        ...
        try {
            ...
            throw new KubernetesApiException(String.format("failed to retrieve pod pool size: %s", e.getMessage()), e);
        }
    }
    ...
    public MachinePool getMachinePool() throws KubernetesApiException {
        ...
        try {
            ...
            throw new KubernetesApiException(String.format("failed to retrieve pod pool members: %s", e.getMessage()),
                    e);
        }
    ...
    public void setDesiredSize(int desiredSize) throws KubernetesApiException {
        ...
        try {
            JsonObject update = JsonUtils.parseJsonString(String.format("{\"spec\": {\"replicas\": %d}}", desiredSize))
                    .getAsJsonObject();
            JsonObject response = this.httpClient.patch(replicationControllerUrl(), update);
            LOG.debug("response: {}", JsonUtils.toPrettyString(response));
        } catch (Exception e) {
            throw new KubernetesApiException(String.format("failed to update pod pool size: %s", e.getMessage()), e);
        }
     ...
     * @return
     */
    private Map<String, String> getPodSelector(JsonObject replicationController) {
        ...
        for (Entry<String, JsonElement> selector : selectors.entrySet()) {
            podSelector.put(selector.getKey(), selector.getValue().getAsString());
        }
     ...
     * @throws IOException
     */
    private JsonObject getReplicationController() throws HttpResponseException, IOException {
    ...
    private String nsQualifiedBaseUrl() {
        return String.format("https://%s:%d/api/v1/namespaces/%s", this.config.getApiServer().getHost(),
                this.config.getApiServer().getPort(), this.config.getPodPool().getNamespace());
    }
