...
public class PodToMachine implements Function<JsonObject, Machine> {
    ...
    public Machine apply(JsonObject pod) {
        ...
        MachineState machineState = new PodStateToMachineState().apply(status.get("phase").getAsString());
        String cloudProvider = "Kubernetes";
        String region = "N/A";
        String machineSize = "N/A";
        DateTime requestTime = UtcTime.parse(metadata.get("creationTimestamp").getAsString());

        // if pod isn't running, no startTime will be available
        String launchTimeValue = getWithDefault(status, "startTime", null);
        DateTime launchTime = launchTimeValue != null ? UtcTime.parse(launchTimeValue) : null;
        ...
        return Machine.builder().id(id).machineState(machineState).cloudProvider(cloudProvider).region(region)
                .machineSize(machineSize).launchTime(launchTime).requestTime(requestTime).publicIp(publicIp)
