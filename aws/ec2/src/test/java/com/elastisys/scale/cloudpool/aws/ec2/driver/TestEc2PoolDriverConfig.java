...
public class TestEc2PoolDriverConfig {
	...
	public void fullConfig() {
		...
		Ec2PoolDriverConfig config = new Ec2PoolDriverConfig("awsAccessKeyId",
				"awsSecretAccessKey", "us-east-1", connectionTimeout,
				socketTimeout);
		config.validate();
	...
	public void withDefaults() {
		...
		assertThat(config.getConnectionTimeout(),
				is(Ec2PoolDriverConfig.DEFAULT_CONNECTION_TIMEOUT));
		assertThat(config.getSocketTimeout(),
				is(Ec2PoolDriverConfig.DEFAULT_SOCKET_TIMEOUT));
	}

	@Test(expected = IllegalArgumentException.class)
	public void missingAwsAccessKeyId() {
		new Ec2PoolDriverConfig(null, "awsSecretAccessKey", "us-east-1")
				.validate();
	}
	...
	public void missingRegion() {
		new Ec2PoolDriverConfig("awsAccessKeyId", "awsSecretAccessKey", null)
				.validate();
	}

	@Test(expected = IllegalArgumentException.class)
	public void illegalConnectionTimeout() {
		new Ec2PoolDriverConfig("awsAccessKeyId", "awsSecretAccessKey",
