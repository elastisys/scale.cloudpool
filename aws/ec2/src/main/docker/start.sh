...
# Logging
LOG_CONFIG=${LOG_CONFIG:-/etc/elastisys/ec2pool/logback.xml}
JUL_CONFIG=${JUL_CONFIG:-/etc/elastisys/ec2pool/logging.properties}
...
if [ "${MULTIPOOL}" = true ]; then
    ${java} ${JVM_OPTS} ${JAVA_OPTS} -cp /opt/elastisys/ec2pool/ec2pool.jar com.elastisys.scale.cloudpool.aws.ec2.server.multipool.Main ${RUNTIME_OPTS} ${SERVER_OPTS} ${AUTH_OPTS}
else
    ${java} ${JVM_OPTS} ${JAVA_OPTS} -jar /opt/elastisys/ec2pool/ec2pool.jar ${RUNTIME_OPTS} ${SERVER_OPTS} ${AUTH_OPTS}
fi
