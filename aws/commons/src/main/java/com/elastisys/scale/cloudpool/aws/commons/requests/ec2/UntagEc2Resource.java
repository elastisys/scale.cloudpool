...
public class UntagEc2Resource extends AmazonEc2Request<Void> {
    ...
    public Void call() {
        for (Tag tag : this.tags) {
            LOG.debug("deleting {}={} tag on instance {}", tag.getKey(), tag.getValue(), this.resourceId);
        }

        DeleteTagsRequest request = new DeleteTagsRequest().withResources(this.resourceId).withTags(this.tags);
