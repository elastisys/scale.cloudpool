...
if [ "${LOG_DIR}" = "" ]; then
    LOG_DIR="/var/log/elastisys/spotpool"
fi

# where runtime state is kept
if [ "${STORAGE_DIR}" = "" ]; then
    STORAGE_DIR="/var/lib/elastisys/spotpool"
fi


logconfig="/etc/elastisys/spotpool/logback.xml"
julconfig="/etc/elastisys/spotpool/logging.properties"
...
# have been set up
authbind --deep ${java} ${JAVA_OPTS} -jar spotpool.jar ${SERVER_OPTS}
