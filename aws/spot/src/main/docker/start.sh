...
# Logging
LOG_CONFIG=${LOG_CONFIG:-/etc/elastisys/spotpool/logback.xml}
JUL_CONFIG=${JUL_CONFIG:-/etc/elastisys/spotpool/logging.properties}
...
STORAGE_DIR=${STORAGE_DIR:-/var/lib/elastisys/spotpool}
...
if [ "${MULTIPOOL}" = true ]; then
    ${java} ${JVM_OPTS} ${JAVA_OPTS} -cp /opt/elastisys/spotpool/spotpool.jar com.elastisys.scale.cloudpool.aws.spot.server.multipool.Main ${RUNTIME_OPTS} ${SERVER_OPTS} ${AUTH_OPTS}
else
    ${java} ${JVM_OPTS} ${JAVA_OPTS} -jar /opt/elastisys/spotpool/spotpool.jar ${RUNTIME_OPTS} ${SERVER_OPTS} ${AUTH_OPTS}
fi
