...
if [ "${MULTIPOOL}" = true ]; then
    ${java} ${JVM_OPTS} ${JAVA_OPTS} -cp /opt/elastisys/awsaspool/awsaspool.jar com.elastisys.scale.cloudpool.aws.autoscaling.server.multipool.Main ${RUNTIME_OPTS} ${SERVER_OPTS} ${AUTH_OPTS}
else
    ${java} ${JVM_OPTS} ${JAVA_OPTS} -jar /opt/elastisys/awsaspool/awsaspool.jar ${RUNTIME_OPTS} ${SERVER_OPTS} ${AUTH_OPTS}
fi
