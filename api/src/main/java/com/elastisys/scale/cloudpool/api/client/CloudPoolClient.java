...
public class CloudPoolClient implements CloudPool {
	 ...
	 *            The port on which the {@link CloudPool} server is listening.
	 */
	public CloudPoolClient(AuthenticatedHttpClient httpClient,
			String cloudPoolHost, int cloudPoolPort) {
	...
	public void configure(JsonObject configuration)
		...
		try {
			...
			request.setEntity(new StringEntity(JsonUtils
					.toPrettyString(configuration),
					ContentType.APPLICATION_JSON));
			this.httpClient.execute(request);
		} catch (HttpResponseException e) {
			if (e.getStatusCode() == Status.BAD_REQUEST.getStatusCode()) {
				throw new IllegalArgumentException(format(
						"failed to set cloud pool config for %s: "
								+ "bad request: %s", url, e.getMessage()), e);
			}
			throw new CloudPoolException(format(
					"failed to set cloud pool config: %s: %s", url,
					e.getMessage()), e);
		} catch (Exception e) {
			throw new CloudPoolException(format(
					"failed to set cloud pool config: %s: %s", url,
	...
	public Optional<JsonObject> getConfiguration() {
		...
		try {
			...
			JsonObject config = responseToJson(this.httpClient.execute(request))
					.getAsJsonObject();
			...
			throw new CloudPoolException(format(
					"failed to get cloud pool config: %s: %s", url,
					e.getMessage()), e);
		} catch (Exception e) {
			throw new CloudPoolException(format(
					"failed to get cloud pool config: %s: %s", url,
	...
	public void start() throws NotConfiguredException {
		...
		try {
			...
			String message = format("failed to start cloud pool %s: %s", url,
					e.getMessage());
			if (e.getStatusCode() == Status.BAD_REQUEST.getStatusCode()) {
			...
			throw new CloudPoolException(format(
					"failed to start cloud pool %s: %s", url, e.getMessage()),
					e);
		}
	}
	...
	public void stop() {
		...
		try {
			...
			String message = format("failed to stop cloud pool %s: %s", url,
					e.getMessage());
			throw new CloudPoolException(message, e);
		}
	}
	...
	public CloudPoolStatus getStatus() {
		...
		try {
			...
			throw new CloudPoolException(format(
					"failed to get cloud pool status: %s: %s", url,
					e.getMessage()), e);
		}
	}
	...
	public MachinePool getMachinePool() throws CloudPoolException,
		...
		try {
			...
			throw new CloudPoolException(format(
					"failed to get cloud pool metadata: %s: %s", url,
					e.getMessage()), e);
		}
	}
	...
	public PoolSizeSummary getPoolSize() throws CloudPoolException,
		...
		try {
			HttpRequestResponse response = this.httpClient.execute(new HttpGet(
					url));
			return responseToObject(response, PoolSizeSummary.class);
		} catch (Exception e) {
			throw new CloudPoolException(format(
					"failed to retrieve pool size from "
							+ "cloud pool at %s: %s", url, e.getMessage()), e);
		}
	}
	...
	public void setDesiredSize(int desiredSize)
		...
		try {
			HttpPost request = new HttpPost(url);
			request.setEntity(new StringEntity(JsonUtils
					.toPrettyString(JsonUtils.toJson(new SetDesiredSizeRequest(
							desiredSize))), ContentType.APPLICATION_JSON));
			this.httpClient.execute(request);
		} catch (HttpResponseException e) {
			if (e.getStatusCode() == Status.BAD_REQUEST.getStatusCode()) {
				throw new IllegalArgumentException(format(
						"failed to set desired size for cloud pool %s: "
								+ "bad request: %s", url, e.getMessage()), e);
			}
			throw new CloudPoolException(format(
					"failed to set desired size for cloud pool %s: %s", url,
					e.getMessage()), e);
		} catch (Exception e) {
			throw new CloudPoolException(format(
					"failed to set desired size for cloud pool %s: %s", url,
	...
	public void terminateMachine(String machineId, boolean decrementDesiredSize)
		...
		try {
			HttpPost request = new HttpPost(url);
			request.setEntity(new StringEntity(JsonUtils
					.toPrettyString(JsonUtils
							.toJson(new TerminateMachineRequest(
			...
			if (e.getStatusCode() == Status.NOT_FOUND.getStatusCode()) {
				throw new NotFoundException(format(
						"failed to terminate machine %s in cloud pool %s: "
								+ "bad request: %s", machineId, url,
						e.getMessage()), e);
			}
			throw new CloudPoolException(format(
					"failed to terminate machine %s in cloud pool %s: %s",
	...
	public void setServiceState(String machineId, ServiceState serviceState)
		...
		try {
			...
			request.setEntity(new StringEntity(JsonUtils
					.toPrettyString(JsonUtils
							.toJson(new SetServiceStateRequest(serviceState))),
					ContentType.APPLICATION_JSON));
			this.httpClient.execute(request);
		} catch (HttpResponseException e) {
			if (e.getStatusCode() == Status.NOT_FOUND.getStatusCode()) {
				throw new NotFoundException(format(
						"failed to set service state for machine %s in cloud pool %s: "
								+ "bad request: %s", machineId, url,
						e.getMessage()), e);
			}
			throw new CloudPoolException(format(
					"failed to set service state for "
							+ "machine %s in cloud pool %s: %s", machineId,
					url, e.getMessage()), e);
		} catch (Exception e) {
			throw new CloudPoolException(format(
					"failed to set service state for "
	...
	public void setMembershipStatus(String machineId,
		...
		try {
			...
			request.setEntity(new StringEntity(JsonUtils
					.toPrettyString(JsonUtils
							.toJson(new SetMembershipStatusRequest(
									membershipStatus))),
					ContentType.APPLICATION_JSON));
			this.httpClient.execute(request);
		} catch (HttpResponseException e) {
			if (e.getStatusCode() == Status.NOT_FOUND.getStatusCode()) {
				throw new NotFoundException(format(
						"failed to set membership status for machine %s in cloud pool %s: "
								+ "bad request: %s", machineId, url,
						e.getMessage()), e);
			}
			throw new CloudPoolException(format(
					"failed to set membership status for "
							+ "machine %s in cloud pool %s: %s", machineId,
					url, e.getMessage()), e);
		} catch (Exception e) {
			throw new CloudPoolException(format(
	...
	public void attachMachine(String machineId) throws NotFoundException,
		...
		try {
			...
			if (e.getStatusCode() == Status.NOT_FOUND.getStatusCode()) {
				throw new NotFoundException(format(
						"failed to attach machine %s to cloud pool %s: "
								+ "bad request: %s", machineId, url,
						e.getMessage()), e);
			}
			throw new CloudPoolException(format("failed to attach "
					+ "machine %s in cloud pool %s: %s", machineId, url,
					e.getMessage()), e);
		} catch (Exception e) {
			throw new CloudPoolException(format("failed to attach "
					+ "machine %s in cloud pool %s: %s", machineId, url,
	...
	public void detachMachine(String machineId, boolean decrementDesiredSize)
		...
		try {
			HttpPost request = new HttpPost(url);
			request.setEntity(new StringEntity(JsonUtils
					.toPrettyString(JsonUtils.toJson(new DetachMachineRequest(
							decrementDesiredSize))),
					ContentType.APPLICATION_JSON));
			this.httpClient.execute(request);
		} catch (HttpResponseException e) {
			if (e.getStatusCode() == Status.NOT_FOUND.getStatusCode()) {
				throw new NotFoundException(format(
						"failed to detach machine %s from cloud pool %s: "
								+ "bad request: %s", machineId, url,
						e.getMessage()), e);
			}
			throw new CloudPoolException(format("failed to detach "
					+ "machine %s from cloud pool %s: %s", machineId, url,
					e.getMessage()), e);
		} catch (Exception e) {
			throw new CloudPoolException(format("failed to detach "
					+ "machine %s from cloud pool %s: %s", machineId, url,
	...
	private String baseUrl() {
		return String.format("https://%s:%d", this.cloudPoolHost,
				this.cloudPoolPort);
	}
	...
	private <T> T responseToObject(HttpRequestResponse response, Class<T> type) {
		return JsonUtils.toObject(
				JsonUtils.parseJsonString(response.getResponseBody()), type);
	}
