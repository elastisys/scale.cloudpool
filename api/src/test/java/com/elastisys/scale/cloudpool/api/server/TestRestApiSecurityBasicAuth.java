...
public class TestRestApiSecurityBasicAuth {
	...
	public void connectWithAuthorizedClient() {
		Client client = RestClients.httpsBasicAuth(AUTHORIZED_CLIENT_USERNAME,
				AUTHORIZED_CLIENT_PASSWORD);
	...
	public void connectWithUnauthorizedClient() {
		Client client = RestClients.httpsBasicAuth(
				UNAUTHORIZED_CLIENT_USERNAME, UNAUTHORIZED_CLIENT_PASSWORD);
		Response response = client.target(getUrl()).request().get();
		assertThat(response.getStatus(), is(UNAUTHORIZED.getStatusCode()));
	}
	...
	public void connectWithWrongRoleClient() {
		Client client = RestClients.httpsBasicAuth(WRONG_ROLE_CLIENT_USERNAME,
				WRONG_ROLE_CLIENT_PASSWORD);
		Response response = client.target(getUrl()).request().get();
