...
public class TestCloudPoolClient {
	...
	public void configureWithIllegalConfig() {
		...
		doThrow(new IllegalArgumentException("bad config!")).when(cloudPool)
				.configure(config);
	...
	public void startBeforeConfigured() {
		doThrow(new NotConfiguredException("no config set!")).when(cloudPool)
				.start();
	...
	public void getStatus() {
		when(cloudPool.getStatus())
				.thenReturn(new CloudPoolStatus(false, true));
	...
	public void terminateInvalidMachine() {
		doThrow(new NotFoundException("not recognized!")).when(cloudPool)
				.terminateMachine("i-X", true);
	...
	public void setServiceStateOnInvalidMachine() {
		doThrow(new NotFoundException("not recognized!")).when(cloudPool)
				.setServiceState("i-X", ServiceState.OUT_OF_SERVICE);
	...
	public void setMembershipStatusOnInvalidMachine() {
		doThrow(new NotFoundException("not recognized!")).when(cloudPool)
				.setMembershipStatus("i-X", MembershipStatus.blessed());
	...
	public void getMetadata() {
		CloudPoolMetadata metadata = new CloudPoolMetadata("AWS/EC2",
