package com.elastisys.scale.cloudpool.juju;
...
public class JujuCloudPool implements CloudPool {
	private static final Logger LOG = LoggerFactory.getLogger(JujuCloudPool.class);
	...
	public static final String DESCRIPTION = "Juju Cloudpool";
	...
	/**
	 * Creates a new {@link JujuCloudPool}.
	 */
	public JujuCloudPool(JujuClient apiClient) {
		this.config = null;
		this.started = false;
		this.jujuClient = apiClient;
	...
	public synchronized void configure(JsonObject configuration) throws IllegalArgumentException, CloudPoolException {
		JujuCloudPoolConfig newConfig = parseAndValidate(configuration);
		...
		try {
			this.jujuClient.configure(newConfig);
		} catch (IOException e) {
			throw new CloudPoolException("Failed to configure cloud pool", e);
		}
	...
	private JujuCloudPoolConfig parseAndValidate(JsonObject configuration) {
		JujuCloudPoolConfig newConfig = null;
		try {
			newConfig = JsonUtils.toObject(configuration, JujuCloudPoolConfig.class);
			newConfig.validate();
		} catch (Exception e) {
	...
	public void start() throws NotConfiguredException {
		...
		this.executor.scheduleWithFixedDelay(new PoolUpdateTask(this.jujuClient, this.desiredSize), interval.getTime(),
	...
	private void tryToDetermineDesiredSize() {
		...
		try {
			...
			this.desiredSize.set(this.jujuClient.getPoolSize().getDesiredSize());
	...
	public MachinePool getMachinePool() throws CloudPoolException, NotStartedException {
		ensureStarted();
		return this.jujuClient.getMachinePool();
	}
	...
	public void setDesiredSize(int desiredSize)
		...
		this.jujuClient.setDesiredSize(desiredSize);
	}
	...
	private static class PoolUpdateTask implements Runnable {
		private final JujuClient apiClient;
		private final AtomicReference<Integer> desiredSize;

		public PoolUpdateTask(JujuClient apiClient, AtomicReference<Integer> desiredSize) {
