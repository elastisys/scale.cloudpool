...
public class TestRetriesConfig {
	...
	public void basicSanity() {
		int maxRetries = 2;
		TimeInterval initialBackoffDelay = new TimeInterval(10L,
				TimeUnit.SECONDS);
		RetriesConfig config = new RetriesConfig(maxRetries,
				initialBackoffDelay);
	...
	public void withNoExponentialBackoff() {
		int maxRetries = 3;
		TimeInterval initialBackoffDelay = new TimeInterval(0L,
				TimeUnit.SECONDS);
		RetriesConfig config = new RetriesConfig(maxRetries,
				initialBackoffDelay);
		assertThat(config.getInitialBackoffDelay(),
				is(new TimeInterval(0L, TimeUnit.SECONDS)));
	}
	...
	public void negativeMaxRetries() {
		new RetriesConfig(-1, new TimeInterval(0L, TimeUnit.SECONDS))
				.validate();
	}
