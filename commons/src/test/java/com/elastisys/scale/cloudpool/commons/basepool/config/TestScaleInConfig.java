...
public class TestScaleInConfig {
	...
	public void basicSanity() {
		ScaleInConfig config = new ScaleInConfig(
				VictimSelectionPolicy.CLOSEST_TO_INSTANCE_HOUR, 500);
		config.validate();
		assertThat(config.getVictimSelectionPolicy(),
		...
		config.validate();
		assertThat(config.getVictimSelectionPolicy(),
				is(VictimSelectionPolicy.NEWEST_INSTANCE));

		config = new ScaleInConfig(VictimSelectionPolicy.OLDEST_INSTANCE, 500);
		config.validate();
		assertThat(config.getVictimSelectionPolicy(),
				is(VictimSelectionPolicy.OLDEST_INSTANCE));
	}
