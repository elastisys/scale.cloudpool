...
public class TestCachingPoolFetcher {
	...
	private static final TimeInterval REACHABILITY_TIMEOUT = new TimeInterval(
			REACHABILITY_TIMEOUT_MINUTES.longValue(), TimeUnit.MINUTES);
	private static final TimeInterval REFRESH_INTERVAL = new TimeInterval(30L,
			TimeUnit.SECONDS);
	private static final PoolFetchConfig FETCH_CONFIG = new PoolFetchConfig(
			new RetriesConfig(3, new TimeInterval(10L, TimeUnit.MILLISECONDS)),
			REFRESH_INTERVAL, REACHABILITY_TIMEOUT);
	...
	public void populateCacheOnFirstCall() {
		when(this.delegate.get(FORCE_REFRESH))
				.thenReturn(pool(machines("i-1", "i-2")));
	...
	public void failIfNoFetchHasBeenSuccessful() {
		when(this.delegate.get(FORCE_REFRESH))
				.thenThrow(new CloudPoolException("api outage"));
	...
	public void persistence() {
		...
		when(this.delegate.get(FORCE_REFRESH))
				.thenReturn(pool(machines("i-1", "i-2", "i-3")));
		assertThat(fetcher.get(FetchOption.FORCE_REFRESH),
				is(pool(machines("i-1", "i-2", "i-3"))));
	...
	public void honorReachabilityTimeoutWithRestoredCache() throws IOException {
		DateTime cachedTimestamp = FrozenTime.now()
				.minusMinutes(REACHABILITY_TIMEOUT_MINUTES + 1);
		MachinePool outdatedPool = new MachinePool(machines("i-1", "i-2"),
				cachedTimestamp);
		File cacheFile = STATE_STORAGE.getCachedMachinePoolFile();
	...
	private MachinePool restore(File machinePoolCacheFile) {
		return JsonUtils.toObject(JsonUtils.parseJsonFile(machinePoolCacheFile),
				MachinePool.class);
	}
