package com.elastisys.scale.cloudadapters.commons.testutils.sshserver;
...
public class ExternalProcessCommand implements Command {
	...
	public void start(Environment env) throws IOException {
		List<String> commandParts = Lists.newArrayList(this.command
				.split("\\s+"));
		...
		try {
			...
			String errorMessage = format("failed to run command: %s",
					e.getMessage());
	...
	private void readProcessOutput(Process process) throws IOException {
		...
		try (InputStream stdout = process.getInputStream();
			...
			while (!done(process) || (stdout.available() > 0)
					|| (stderr.available() > 0)) {
