...
import static com.elastisys.scale.cloudadapters.commons.resizeplanner.ResizePlanTestUtils.makeMachine;
import static com.elastisys.scale.cloudadapters.commons.resizeplanner.ResizePlanTestUtils.makePool;
import static com.elastisys.scale.cloudadapters.commons.resizeplanner.ResizePlanTestUtils.nowOffset;
import static com.elastisys.scale.cloudadapters.commons.resizeplanner.ResizePlanTestUtils.terminationMarked;
import static com.elastisys.scale.cloudadapters.commons.scaledown.VictimSelectionPolicy.CLOSEST_TO_INSTANCE_HOUR;
...
public class TestResizePlanner {
	...
	public void testGetEffectiveSize() {
		...
		// pool size: 3 (1 REQUESTED, 1 PENDING, 1 RUNNING), termination queue
		// size: 0
		...
		// pool size: 1, termination queue size: 1
		...
		// pool size: 2, termination queue size: 1
		pool = makePool(UtcTime.now(), asList(pending, running));
		termq = new TerminationQueue();
		...
		// pool size: 2, termination queue size: 2
		pool = makePool(UtcTime.now(), asList(pending, running));
		termq = new TerminationQueue();
		termq.add(new ScheduledTermination(pending, nowOffset(1500)));
	...
	public void testGetEffectiveSizeWithInstancesInTerminalState() {
		...
		MachinePool pool = makePool(
				UtcTime.now(),
				asList(pending, running, requested, terminated, terminating,
						rejected));
	...
	public void scaleUpWithEmptyTerminationQueue() {
		...
		// pool size: 3
		MachinePool pool = makePool(now(), asList(requested, pending, running));
		planner = new ResizePlanner(pool, new TerminationQueue(),
				OLDEST_INSTANCE, INSTANCE_HOUR_MARGIN);
		assertThat(planner.getEffectiveSize(), is(3));
		// 3 -> 4
	...
	public void scaleUpWithNonEmptyTerminationQueue() {
		...
		// pool size: 3, termination queue size: 1
		MachinePool pool = makePool(UtcTime.now(),
				asList(pending, running, requested));
		TerminationQueue termq = new TerminationQueue();
	...
	public void testStayPutRequest() {
		...
		// pool size: 2, termination queue size: 1
		pool = makePool(UtcTime.now(), asList(running, pending));
		termq = new TerminationQueue();
		termq.add(new ScheduledTermination(pending, nowOffset(1500)));
		planner = new ResizePlanner(pool, termq, OLDEST_INSTANCE,
				INSTANCE_HOUR_MARGIN);
	...
	public void scaleDownWithEmptyTerminationQueue() {
		...
		// pool size: 1, termination queue size: 0
		MachinePool pool = makePool(UtcTime.now(), asList(pending));
		TerminationQueue termq = new TerminationQueue();
		ResizePlanner planner = new ResizePlanner(pool, termq, OLDEST_INSTANCE,
				INSTANCE_HOUR_MARGIN);
		assertThat(planner.getEffectiveSize(), is(1));
	...
	public void scaleDownWithInstancesInRequestedState() {
		...
		MachinePool pool = makePool(UtcTime.now(),
				asList(pending, running, requested1, requested2));
		TerminationQueue termq = new TerminationQueue();
		ResizePlanner planner = new ResizePlanner(pool, termq, OLDEST_INSTANCE,
				INSTANCE_HOUR_MARGIN);
	...
	public void scaleDownWithNonEmptyTerminationQueue() {
		...
		// pool size: 3, termination queue: [machine1]
		MachinePool pool = makePool(UtcTime.now(),
				asList(machine1, machine2, machine3));
		TerminationQueue termq = new TerminationQueue();
	...
	public void scaleDownWithInstancesInTerminalStates() {
		Machine rejected = makeMachine(1, nowOffset(-3600),
				MachineState.REJECTED);
		Machine terminating = makeMachine(2, nowOffset(-3400),
				MachineState.TERMINATING);
		Machine terminated = makeMachine(3, nowOffset(-3200),
				MachineState.TERMINATED);
		Machine running = makeMachine(2, nowOffset(-1800), MachineState.RUNNING);

		// pool size: 1 (only 1 machine in a non-terminal state)
		MachinePool pool = makePool(UtcTime.now(),
				asList(rejected, terminating, terminated, running));
		TerminationQueue termq = new TerminationQueue();
		ResizePlanner planner = new ResizePlanner(pool, termq, OLDEST_INSTANCE,
				INSTANCE_HOUR_MARGIN);
		assertThat(planner.getEffectiveSize(), is(1));
	...
	public void scaleDownWithEmptyVictimCandidateSet() {
		...
		MachinePool pool = makePool(UtcTime.now(),
				asList(victim1, victim2, terminated));
		TerminationQueue termq = new TerminationQueue();
	...
	public void verifyThatVictimSelectionPolicyIsHonored() {
		...
		ResizePlanner planner = new ResizePlanner(pool, termq, OLDEST_INSTANCE,
				INSTANCE_HOUR_MARGIN);
		assertThat(terminationMarked(planner.calculateResizePlan(2)),
				is(asList(machine1)));
		assertThat(terminationMarked(planner.calculateResizePlan(1)),
				is(asList(machine1, machine2)));
		assertThat(terminationMarked(planner.calculateResizePlan(0)),
				is(asList(machine1, machine2, machine3)));

		// victim selection policy: newest instance
		planner = new ResizePlanner(pool, termq, NEWEST_INSTANCE,
				INSTANCE_HOUR_MARGIN);
		assertThat(terminationMarked(planner.calculateResizePlan(2)),
				is(asList(machine3)));
		assertThat(terminationMarked(planner.calculateResizePlan(1)),
				is(asList(machine3, machine2)));
		assertThat(terminationMarked(planner.calculateResizePlan(0)),
				is(asList(machine3, machine2, machine1)));

		// victim selection policy: closest to instance hour
		planner = new ResizePlanner(pool, termq, CLOSEST_TO_INSTANCE_HOUR,
				INSTANCE_HOUR_MARGIN);
		assertThat(terminationMarked(planner.calculateResizePlan(2)),
				is(asList(machine2)));
		assertThat(terminationMarked(planner.calculateResizePlan(1)),
				is(asList(machine2, machine3)));
		assertThat(terminationMarked(planner.calculateResizePlan(0)),
				is(asList(machine2, machine3, machine1)));
	}
	...
	public void verifyImmediateTermination() {
		...
		assertThat(plan.getToTerminate().get(0).getTerminationTime(),
				is(UtcTime.now()));
		assertThat(plan.getToTerminate().get(1).getTerminationTime(),
				is(UtcTime.now()));
		assertThat(plan.getToTerminate().get(2).getTerminationTime(),
				is(UtcTime.now()));
	}
	...
	public void verifyLateTermination() {
		...
		assertThat(terminationMarked(plan),
				is(asList(machine1, machine2, machine3)));
		assertThat(plan.getToTerminate().get(0).getTerminationTime(),
				is(UtcTime.parse("2012-03-26T13:40:30Z")));
		assertThat(plan.getToTerminate().get(1).getTerminationTime(),
				is(UtcTime.parse("2012-03-26T13:20:00Z")));
		assertThat(plan.getToTerminate().get(2).getTerminationTime(),
				is(UtcTime.parse("2012-03-26T13:29:00Z")));
		...
		assertThat(plan.getToTerminate().get(1).getTerminationTime(),
				is(UtcTime.parse("2012-03-26T13:22:00Z")));
		assertThat(plan.getToTerminate().get(2).getTerminationTime(),
				is(UtcTime.parse("2012-03-26T13:29:00Z")));
		...
		assertThat(terminationMarked(plan),
				is(asList(machine1, machine2, machine3)));
		assertThat(plan.getToTerminate().get(0).getTerminationTime(),
				is(UtcTime.parse("2012-03-26T13:35:30Z")));
		assertThat(plan.getToTerminate().get(1).getTerminationTime(),
				is(UtcTime.parse("2012-03-26T13:15:00Z")));
		assertThat(plan.getToTerminate().get(2).getTerminationTime(),
				is(UtcTime.parse("2012-03-26T13:24:00Z")));
	}
	...
	public void verifyThatRequestedStateInstancesAreTerminatedImmediately() {
		...
		Machine running = makeMachine(2, nowOffset(-1800), MachineState.RUNNING);
		Machine requested1 = makeMachine(3, null, MachineState.REQUESTED);
		Machine requested2 = makeMachine(4, nowOffset(-1800),
	...
	public void createWithNullPool() {
		new ResizePlanner(null, new TerminationQueue(),
				VictimSelectionPolicy.CLOSEST_TO_INSTANCE_HOUR, 600);
	}
	...
	public void createWithNegativeInstanceHourMargin() {
		new ResizePlanner(makePool(), new TerminationQueue(),
				VictimSelectionPolicy.CLOSEST_TO_INSTANCE_HOUR, -1);
	}

	@Test(expected = IllegalArgumentException.class)
	public void createWithTooLongInstanceHourMargin() {
		new ResizePlanner(makePool(), new TerminationQueue(),
				VictimSelectionPolicy.CLOSEST_TO_INSTANCE_HOUR, 3600);
	}
