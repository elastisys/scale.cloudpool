...
# Logging
LOG_CONFIG=${LOG_CONFIG:-/etc/elastisys/azurepool/logback.xml}
JUL_CONFIG=${JUL_CONFIG:-/etc/elastisys/azurepool/logging.properties}
# log destination dir for default LOG_CONFIG
LOG_DIR=${LOG_DIR:-/var/log/elastisys/azurepool}
mkdir -p ${LOG_DIR}
...
STORAGE_DIR=${STORAGE_DIR:-/var/lib/elastisys/azurepool}
...
if [ "${MULTIPOOL}" = true ]; then
    ${java} ${JVM_OPTS} ${JAVA_OPTS} -cp /opt/elastisys/azurepool/azurepool.jar com.elastisys.scale.cloudpool.azure.server.multipool.Main ${RUNTIME_OPTS} ${SERVER_OPTS} ${AUTH_OPTS}
else
    ${java} ${JVM_OPTS} ${JAVA_OPTS} -jar /opt/elastisys/azurepool/azurepool.jar ${RUNTIME_OPTS} ${SERVER_OPTS} ${AUTH_OPTS}
fi
