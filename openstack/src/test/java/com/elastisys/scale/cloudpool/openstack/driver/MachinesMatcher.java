...
public class MachinesMatcher extends TypeSafeMatcher<List<Machine>> {
    ...
    public void describeTo(Description description) {
        description.appendText("a list of machines with ids " + this.expectedMachineIds);
    }

    @Override
    public boolean matchesSafely(List<Machine> machines) {
        if (this.expectedMachineIds.size() != machines.size()) {
            LOG.warn("expected {} machines, was: {}", this.expectedMachineIds.size(), machines.size());
            return false;
        }
        for (Machine machine : machines) {
            if (!this.expectedMachineIds.contains(machine.getId())) {
                LOG.warn("machine id {} was not among expected ones: {}", machine.getId(),
                        this.expectedMachineIds.size());
                return false;
            }
        }
        return true;
    ...
    public static <T> Matcher<List<Machine>> machines(String... expectedIds) {
        return new MachinesMatcher(Arrays.asList(expectedIds));
    }
}
